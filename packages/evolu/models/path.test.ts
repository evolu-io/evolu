import { eqPath, byDirection, byContains } from './path';
...
test('byContains', () => {
  expect(byContains.compare([0], [0])).toBe(0);
  expect(byContains.compare([0], [0, 0])).toBe(1);
  expect(byContains.compare([0], [0, 1])).toBe(1);
  expect(byContains.compare([0, 1], [0])).toBe(-1);
  expect(byContains.compare([0], [1])).toBe(-1);
  expect(byContains.compare([1], [0])).toBe(-1);
});
